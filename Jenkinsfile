        ...
        stage('checkout-and-test') {
           ...
           /* // Read payload which is a submitted JSON request from github and write to temp file
            ...
            if(true) {*/
                ...
                -l BRANCH=$branch \
                | oc create -f -"""*/
            sh """oc process -f nodejs-mongo-jenkinspipe.json -p NAME=$branch -p SOURCE_REPOSITORY_URL=https://github.com/ttaylorxv/tickHW.git -p SOURCE_REPOSITORY_REF=$branch -p DATABASE_NAME=$branch -p DATABASE_SERVICE_NAME=$branch-mongodb -l BRANCH=$branch """
           // } else {
