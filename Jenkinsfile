    ...
    node {
        stage('checkout-and-test') {
          ...
          //
            ...
            sh """oc process -f \$(pwd)/nodejs-mongo-jenkinspipe.json -p NAME=$branch -p SOURCE_REPOSITORY_URL=https://github.com/ttaylorxv/tickHW.git -p SOURCE_REPOSITORY_REF=$branch -p DATABASE_NAME=$branch -p DATABASE_SERVICE_NAME=$branch-mongodb -l BRANCH=$branch | oc create -f -"""
           // } else {
